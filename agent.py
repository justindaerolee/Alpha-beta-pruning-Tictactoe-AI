...
# A Node data structure is used to implement the Min-Max search algorithm using
# alpha-beta pruning in order to search to greater depths. Nodes are called
# MinMaxNodes and hold the board state, the move made and heuristic score of the
# board. It also holds a pointer to the previous board state Node.

# The score attribute of the MinMaxNode is an array of scores that represents the
# heuristic of each subgrid of the 9-board tictactoe. The score of a subgrid is
# calculated in checkGrid() and works by summing a heuristic of each of the possible
# lines in a subgrid. The score of the line is calculated in checkLine(), where
# 3 in a row gives a score of 1000, 2 pieces in a line without any opponent pieces
# gives a score of 10, and a single piece in a line without any opponent pieces
# gives a score of 1. Otherwise, the line is full or empty which yields no advantage

# Originally the heuristic was calculated at the leaves of the search which resulted
# in 9 calls to checkGrid() for each leaf node. However, we implemented a design
# change which only makes 9 calls to checkGrid() at the top of the search. For each
# node further generated by getChildren(), we then only made a single call to
# checkGrid() for the subgrid that changed. This significantly improved our ability
# to search deeper into the tree. Previously the search depth was only able to
# consistently reach depth 5, however with this we were able to reach one deeper

# The heuristic was then altered to include avoiding subgrids where the opponent
# had an advantage, further increasing our ability to prune more branches of the
# search tree.

# Finally we implemented the ability to increase search depth as the game went on
    ...
    def getChildren(self, targetBoard, num):
        ...
        for i in range(1,10):
            if (self.fullBoard[targetBoard][i] == 0):
                ...
                # Avoid subgrid that opponent is winning and prioritise subgrids
                # player is winning
                if scores2[i] < -10:
                    scores2[i] -= 50
                elif scores2[i] > 10:
...
def parse(string):
    ...
    if command == "second_move":
        ...
        place(curr, bestMove, PLAYER)
        MOVE += 1
        # Increase depth of search by 1 every 7th move
        if MOVE % 7 == 0:
            DEPTH += 1
